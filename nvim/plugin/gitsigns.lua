...
vim.schedule(function()
  require('gitsigns').setup {
    ...
    on_attach = function(bufnr)
      ...
      map('n', '<leader>gl', function() gs.blame_line { full = true } end, { desc = 'git [h] [b]lame line (full)' })
      map('n', '<leader>g~l', gs.toggle_current_line_blame, { desc = '[g]it toggle current [l]ine [b]lame' })
      map('n', '<leader>gd.', gs.diffthis, { desc = 'git [h] [d]iff this' })
      map('n', '<leader>gdh', function() gs.diffthis('~') end, { desc = 'git [h] [D]iff ~' })
      map('n', '<leader>g~d', gs.toggle_deleted, { desc = 'git [t]oggle [d]eleted' })
      -- Text object
